...
import { BotCalculatorService } from "./bot-calculator";
...
export class BotCrawler {
    ...
    botFirstTurn() {
        for (let rackIndex = 0; rackIndex < this.bot.letterRack.length; rackIndex++) {
            ...
            if (startingLetter !== JOKER_CHAR) {
                ...
                initialWord.isVertical = Math.random() > 0.5;
    ...
    private possibleWordsValidator(possiblyValidWords: ValidWord[]) {
        for (const wordData of possiblyValidWords) {
            ...
            const validWords = this.wordValidator.listOfValidWord(this.bot.game.actionCompiler.translate({
                type: OnlineActionType.Place,
                letters: wordData.word,
                placementSettings: placement
            }, this.bot) as PlaceLetter, this.bot.game.board.grid, this.bot.game.gameToken)
            const wordIsValid = validWords.length > EMPTY;
            if (!wordIsValid) {
                continue;
            }
            const words = validWords.map((validWord) => validWord.letters);
            const pointEstimation = BotCalculatorService.testPlaceLetterCalculation(wordData.numberOfLettersPlaced, words);
