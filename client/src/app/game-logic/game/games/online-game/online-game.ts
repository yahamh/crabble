...
export class OnlineGame extends Game {
    ...
    constructor(
        ...
        public arePowerEnabled: boolean,
        ...
        this.playerTurnsAi$$ = this.onlineSocket.playerTurnsAiSubject$.subscribe((playerName: string) => {
            this.turnPlayerToAi(playerName);
            this.playerTurnsAiSubject.next(playerName);
        });
    }

    getLetterList(): Letter[] {
    ...
    protected updateClient(gameState: GameState) {
        ...
        this.updateTimePerTurn(gameState);
    }

    private turnPlayerToAi(playerName: string) {
    ...
    private updateBoard(gameState: GameState) {
        this.boardService.board.updateGrid(gameState.grid);
    }
    ...
    private updateLettersRemaining(gameState: GameState) {
        ...
        this.letterList = gameState.letterList;
    }
    ...
    private updatePlayers(gameState: GameState) {
        for (const lightPlayer of gameState.players) {
            ...
            player.cards = lightPlayer.cards ?? [];
            player.wordsBeforeCard = lightPlayer.wordsBeforeCard ?? 3;
        ...
        this.observers = gameState.observers;
    }
    ...
    private updateTimePerTurn(gameState: GameState) {
        this.timePerTurn = gameState.turnTime;
    }
}
