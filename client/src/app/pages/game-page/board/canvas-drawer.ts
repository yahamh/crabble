...
export class CanvasDrawer {
    ...
    drawGrid(board: Board, fontsize: number): void {
        ...
        for (let i = 0; i < board.grid.length; i++) {
            for (let j = 0; j < board.grid.length; j++) {
                if(board.grid[i][j].firstMove) {
                    this.drawFirstMove({x: j, y: i})
                }
    ...
    private drawFirstMove(tilePos: Vec2) {
        const pos = this.tilePositionToCoord(tilePos.x, tilePos.y);
        ...
        this.canvas.fillStyle = INDICATOR_ENEMY;
        
        this.canvas.beginPath();
        this.canvas.moveTo(pos.x + 0.5 * this.tileSize, pos.y);
        ...
        this.canvas.lineTo(pos.x, pos.y + 0.5 * this.tileSize);
        this.canvas.fill();
    }
