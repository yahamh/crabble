...
export class NewGamePageComponent {
    ...
    openMultiGameForm() {
        ...
        dialogRef.afterClosed().subscribe((formOnline: GameSettings) => {
            ...
            const onlineGameSettings: OnlineGameSettingsUI = {
                gameMode: formOnline.arePowersEnabled ? GameMode.Power : GameMode.Classic,
                timePerTurn: formOnline.timePerTurn,
                players: [formOnline.playerName],
                randomBonus: formOnline.randomBonus,
                dictTitle: formOnline.dictTitle,
                dictDesc: formOnline.dictDesc,
                capacity: formOnline.capacity,
                isPrivate: formOnline.isPrivate,
                password: formOnline.password,
                cards: formOnline.cards,
            };
            this.socketHandler.createGameMulti(onlineGameSettings);
            const username = formOnline.playerName;
            this.openWaitingForPlayer(username, onlineGameSettings.capacity);
        });
    }

    openWaitingForPlayer(username: string, capacity: number) {
        ...
        secondDialogRef.componentInstance.capacity = capacity;
        secondDialogRef.componentInstance.addPlayer(username);
        secondDialogRef.afterOpened().subscribe(() => {
            this.socketHandler.pendingGameId$.subscribe((id) => {
                secondDialogRef.componentInstance.gameId = id;
            });
            ...
            this.socketHandler.players$.subscribe((players) => {
                if (!players) {
                    return;
                }
                if (players.length == 0) {
                ...
                secondDialogRef.componentInstance.setPlayers(players);
            });
        });
    ...
    private createGame(gameSettings: GameSettings): BehaviorSubject<boolean> {
        return undefined as unknown as BehaviorSubject<boolean>
    }
